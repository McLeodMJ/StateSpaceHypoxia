...
p.filter.rec.vector.list <- function(dat, fish, hypox_a, hypox_b, fi, cv_q, sigma_p, rec_var){
  ...
  Avg.likel <- rep(NA, time-1) #average of the ftmp / exclude one because at time t it is NA
  ...
  for (t in 2:time){
    ...
    likel[which(!is.finite(likel))] <- -1e308  # smallest non-infinite value
  ...
  Avg.likel[which(!is.finite(Avg.likel))] <- -1e308  # smallest non-infinite value
