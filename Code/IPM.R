...
kernmat <- function(x, params, timestep){
  ...
  nsim = 200
  ######## #3D array to account for variabilty  ########
  sims <- array(rep(NA, nsim * nsim * nsim), c(nsim, nsim, nsim)) #3D array for sim. pop. = [iter, row, col]
  # runs the Linf iterations and average across all to get a single 200x200 matrix
  
  sims[1,,] <- t(matrix(x, nrow=length(x), ncol=length(x)))
  sims[,,1] <- matrix(x, nrow=length(x), ncol=length(x))
  
  # Survival part of kernel
  # example: declining mortality with size (from Botsford et al. 1994)
  m <- 5.81 * exp(-0.043 * sims[1,,])
  ...
  g.tmp <-  Len_sims[,,1] - (Len_sims[,,1] - Len_sims[,1,]) * exp(-params$k * timestep) # use those two matrices to get the range of possible growth rates, as a function of X
  ...
  pg <- dnorm(sims[,,1], mean = Len_sims[1,,], sd = 2 * dx) # use dnorm to get the distribution of growth rates (using an arbitrarily small sd)
  ...
  Rvec = dnorm(sims[,,1], params$Rmean, params$Rsd) # size distribution of recruits
  Fec = params$Fec_constant * sims[1,,] ^params$Fec_exponent # you can use values of 1 and 3 for these... 
...
# Create parameters:
params = list(k = 0.3, Linf = 100, Lvar = 10.1/100, Rmu = 60, # Lvar is CV of Linf (SD/mean)
              Rmean = 5, Rsd = 1,Fec_constant = 1, Fec_exponent = 3) # mean & sd of size of new recruits
# rates per year 
# linf cm
...
# can you estiamte the fishing rate getting info from edgthe acccounting for hypoxia 


# IPM integration parameters:
meshsize = 200
meshmin = 0
meshmax = params$Linf * 2
...
Rvec <- dnorm(x, mean = params$Rmean, sd = params$Rsd)
# this should integrate to unity (1) - does it?

# Create the kernel
K <- kernmat(x, params, timestep = 1/36) # in 1/36 of a year time step

# Initialize the model:
time = 1000
N = matrix(0, nrow = meshsize, ncol = time)
N[,1] <- Rvec * params$Rmu # initialize with one pulse of new recruits

# Run the model
for (t in 2:time){
  N[,t] <- K %*% N[,t-1] * dx  + Rvec * params$Rmu # midpoint rule integration
}
