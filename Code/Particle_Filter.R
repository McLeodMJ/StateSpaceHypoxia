...
Selex <- read.csv("../Data/Sp.selex.csv")
WClen <- Selex$Dsole # selectivity for Dsole
...
Pfil_Out <- p.filter(Pop_IPM_eq$Pop.matrix, Pop_var, "Dsole", Dsole_f, NA, 200, 100, 100, 0.01, 0.1, 0.1) # particle filter using 1st IPM runs data to initialize the model and 2nd IPM run as the Nact data
