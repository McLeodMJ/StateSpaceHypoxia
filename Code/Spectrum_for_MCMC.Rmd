...
```{r}
  ...
  F0 <- abs((fft(bad_yr$dt2[1:nrow(bad_yr)/10]) ) ) / length(bad_yr$dt2[1:nrow(bad_yr)/10])
  F0 = F0[1:length(f)]
 ...
 fft_sims <- do.call(data.frame,lapply(fft_sims, function(x) replace(x, is.infinite(x),NA)))
  ...
  # simulated data for 10-yrs (these are shown as continuous but really gap in ts for off-season months)
